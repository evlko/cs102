...
def ego_network(
    ...
    for mutual in get_mutual(user_id, target_uids=friends):
        for common in tp.cast(MutualFriends, mutual)["common_friends"]:
            results.append((tp.cast(MutualFriends, mutual)["id"], common))
    return results
